...
void number_theory_functions::negate(uint64_t &r, uint64_t a) {
	r = -a;
}

void number_theory_functions::addMod(uint64_t &r, uint64_t a, uint64_t b, uint64_t m) {
	r = (a + b) % m;
}

void number_theory_functions::subMod(uint64_t &r, uint64_t a, uint64_t b, uint64_t m) {
	r = b % m;
	r = (a + m - r) % m;
}

void number_theory_functions::mulMod(uint64_t &r, uint64_t a, uint64_t b, uint64_t m) {
	unsigned __int128 mul = static_cast<unsigned __int128>(a % m) * (b % m);
	mul %= static_cast<unsigned __int128>(m);
	r = static_cast<uint64_t>(mul);
}

void number_theory_functions::mulModBarrett(uint64_t& r, uint64_t a, uint64_t b, uint64_t p, uint64_t pr, long twok) {
	unsigned __int128 mul = static_cast<unsigned __int128>(a % p) * (b % p);
	modBarrett(r, mul, p, pr, twok);
}

void number_theory_functions::modBarrett(uint64_t &r, uint64_t a, uint64_t m, uint64_t mr, long twok) {
...
void number_theory_functions::modBarrett(uint64_t &r, unsigned __int128 a, uint64_t m, uint64_t mr, long twok) {
	uint64_t atop, abot;
...
uint64_t number_theory_functions::invMod(uint64_t x, uint64_t m) {
	uint64_t temp = x % m;
	if (gcd(temp, m) != 1) {
...
uint64_t number_theory_functions::powMod(uint64_t x, uint64_t y, uint64_t modulus) {
	uint64_t res = 1;
	while (y > 0) {
		if (y & 1) {
...
uint64_t number_theory_functions::inv(uint64_t x) {
	return pow(x, static_cast<uint64_t>(-1));
}

uint64_t number_theory_functions::pow(uint64_t x, uint64_t y) {
	uint64_t res = 1;
	while (y > 0) {
		if (y & 1) {
			res *= x;
...
uint32_t number_theory_functions::bitReverse(uint32_t x) {
	//cout << "bitReverse\n";
...
uint64_t number_theory_functions::gcd(uint64_t a, uint64_t b) {
	if (a == 0) {
		return b;
	}
	return gcd(b % a, a);
...
long number_theory_functions::gcd(long a, long b) {
	if (a == 0) {
		return b;
	}
	return gcd(b % a, a);
...
void number_theory_functions::findPrimeFactors(set<uint64_t> &s, uint64_t number) {
	while (number % 2 == 0) {
		s.insert(2);
		number /= 2;
	}
...
uint64_t number_theory_functions::findPrimitiveRoot(uint64_t modulus) {
	set<uint64_t> s;
...
uint64_t number_theory_functions::findMthRootOfUnity(uint64_t M, uint64_t mod) {
    uint64_t res;
...
bool number_theory_functions::primeTest(uint64_t p) {
	if(p < 2) return false;
